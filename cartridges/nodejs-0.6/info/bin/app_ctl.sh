...
function _is_node_service_running() {
    if [ -f $OPENSHIFT_NODEJS06_DIR/run/node.pid ]; then
        node_pid=$( cat $OPENSHIFT_NODEJS06_DIR/run/node.pid 2> /dev/null )
        myid=$( id -u )
...
function _start_node_service() {
    ...
    if [ -f $OPENSHIFT_NODEJS06_DIR/run/stop_lock -o idle = "$_state" ]; then
    ...
    envf="$OPENSHIFT_NODEJS06_DIR/conf/node.env"
    ...
    popd > /dev/null
    if [ $ret -eq 0 ]; then
        echo "$npid" > "$OPENSHIFT_NODEJS06_DIR/run/node.pid"
...
function _stop_node_service() {
    ...
    if [ -n "$node_pid" ]; then
        ...
        rm -f $OPENSHIFT_NODEJS06_DIR/run/node.pid
