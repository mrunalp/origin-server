...
then
    ...
    rm -rf "${OPENSHIFT_PHP53_DIR}"/phplib/pear/*
    mkdir -p "${OPENSHIFT_PHP53_DIR}"/phplib/pear/{docs,ext,php,cache,cfg,data,download,temp,tests,www}
    pear -c ~/.pearrc config-set php_ini "${OPENSHIFT_PHP53_DIR}/conf/php.ini"
fi
