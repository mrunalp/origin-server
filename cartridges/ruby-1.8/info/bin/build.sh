...
then
  USED_BUNDLER=false
  if [ -d ${OPENSHIFT_RUBY18_CART_DIR}tmp/.bundle ]
  then
    USED_BUNDLER=true
  fi
  ...
  then
    ...
    mv ${OPENSHIFT_RUBY18_CART_DIR}tmp/.bundle ${OPENSHIFT_REPO_DIR}
    if [ -d ${OPENSHIFT_REPO_DIR}vendor ]
    then
      mv ${OPENSHIFT_RUBY18_CART_DIR}/tmp/vendor/bundle ${OPENSHIFT_REPO_DIR}vendor/
    else
      mv ${OPENSHIFT_RUBY18_CART_DIR}tmp/vendor ${OPENSHIFT_REPO_DIR}
    fi
    rm -rf ${OPENSHIFT_RUBY18_CART_DIR}tmp/.bundle ${OPENSHIFT_RUBY18_CART_DIR}tmp/vendor
  fi
